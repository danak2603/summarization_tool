from app.data_loader import (parse_pubmed_file_filtered,
...
from app.retrieval import (step_back_and_extract_topics,
...
def generate_summary(user_role: str, user_question: str) -> str:
    ...
    filtered_articles_pubmed = filter_pubmed_articles_by_topics(parsed_articles_pubmed, expand_topics) # or expand_topics

    pmc_dirs = [os.path.join("data", d) for d in os.listdir("data") if
                d.lower().startswith("pmc") and os.path.isdir(os.path.join("data", d))]
    articles_pmc = []
...
if __name__ == "__main__":
    user_role = input("Enter your job title: ")
    user_question = input("Enter your research question: ")

    summary = generate_summary(user_role=user_role, user_question=user_question)
